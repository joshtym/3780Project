...
{	
	...
	vector<Message> messageListOfNonExistantClients;
			...
			for (int i = 1; i < 6; i++)
				if( (dummy.distanceVector[i] != 0 ) && (i != serverID) )
					if ((dummy.distanceVector[i] + serverInfo[serverID].distanceVector[serverLeft])  < serverInfo[serverID].distanceVector[i])
					{
						...
						serverInfo[serverID].throughWhichRouter[i] = serverLeft;
					}
			...
			for (int i = 1; i < 6; i++) 
				if((dummy.distanceVector[i] != 0 ) && (i != serverID))
					if ((dummy.distanceVector[i] + serverInfo[serverID].distanceVector[serverRight])  < serverInfo[serverID].distanceVector[i])
					{
						...
						serverInfo[serverID].throughWhichRouter[i] = serverRight;
					}
		...
		switch (m.mType)
		{
			case ACK:
				...
				else
				{
					...
					if (result > 0)
					{
						cout << "Receieved ACK packet from " << m.source << " destined for " << m.dest << " and we're sending it on through the shortest path of " << serverInfo[serverID].throughWhichRouter[result] << "." << endl;
						
						if (serverInfo[serverID].throughWhichRouter[result] == serverLeft)
							n = sendto(sock, &m, sizeof(struct Message), 0,(struct sockaddr *)&server2,fromlen);
						else
							n = sendto(sock, &m, sizeof(struct Message), 0,(struct sockaddr *)&server1,fromlen);
					...
					{
						cout << "Couldn't find any client on the servers with that name. Storing for now" << endl;
						messageListOfNonExistantClients.push_back(m);
					}
			...
			case SEND:
				...
				else
				{
					...
					if (result > 0)
					{
						cout << "Receieved SEND packet from " << m.source << " destined for " << m.dest << " and we're sending it on through the shortest path of " << serverInfo[serverID].throughWhichRouter[result] << "." << endl;
